...
const EditRequisition: React.FC = () => {
  ...
  const fetchRequisition = async () => {
    try {
      ...
      setFormData({
        ...
        department_id: req.department_id.toString(),
        requested_date: req.requested_date.split('T')[0],
        status: req.status,
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await RequisitionAPI.updateRequisition(Number(id), {
        ...formData,
  ...
  return (
    <DashboardLayout
      title="Edit Requisition"
      subtitle="Update requisition details"
      showSearch={false}
    >
